...
/**
 * Suppression-safe frozen program outcome aggregate rows.
 *
 * Mutable program, cohort, outcome, framework, and policy records are only
 * LEFT-joined fallbacks. Stored snapshot IDs provide filter identity and
 * immutable companion snapitem payloads provide historical labels.
 *
...
final class program_aggregates extends secured_datasource {
    ...
    protected function initialise_source(): void {
        $entity = new report_record(
            ...
            new lang_string('report_source_program_aggregates', 'local_outcomemap')
        );
        ...
        $policycapture = database::generate_alias('policycapture');
        ...
        $this->add_join("LEFT JOIN {local_outcomemap_snapitem} {$policycapture}
                             ON {$policycapture}.snapshotid = {$item}.snapshotid
                            AND {$policycapture}.itemtype = 'policy_version'
                            AND {$policycapture}.sourceid = {$snapshot}.policyid");
        ...
        $this->add_join("LEFT JOIN {local_outcomemap_policy} {$policy}
                             ON {$policy}.id = {$snapshot}.policyid");
