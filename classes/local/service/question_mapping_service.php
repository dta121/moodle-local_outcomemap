...
final class question_mapping_service extends base_service {
    ...
    /**
     * Build a qbank mapped-state filter against the exact question-version alias.
     *
     * The returned fragment expects the outer question-bank query to expose the
     * selected exact version as `qv.id`. Keeping this SQL here preserves table
     * ownership while allowing the companion plugin to participate in core's
     * condition API.
     *
    ...
    public static function build_mapped_filter_query(\context $context, bool $mapped): array {
        require_capability('local/outcomemap:viewdefinitions', $context);
        $exists = 'EXISTS (SELECT 1 FROM {local_outcomemap_qmap} ommap'
            . ' WHERE ommap.questionversionid = qv.id)';
        return [$mapped ? $exists : 'NOT ' . $exists, []];
    }

    /**
     * Build a qbank outcome-code filter against the exact question-version alias.
     *
     * @param \context $context Question-bank context authorizing mapping metadata access.
    ...
    public static function build_outcome_filter_query(\context $context, array $values, int $jointype): array {
        ...
        $allowedjointypes = [
            ...
            \core\output\datafilter::JOINTYPE_ALL,
            \core\output\datafilter::JOINTYPE_NONE,
        ];
        if (!in_array($jointype, $allowedjointypes, true)) {
            $jointype = \core\output\datafilter::JOINTYPE_ANY;
        }

        $fullcode = $DB->sql_concat('omf.code', "'.'", 'omi.code');
        $fragments = [];
        $params = [];
        foreach (array_values($cleanvalues) as $index => $value) {
            $codeparam = 'omcode' . $index;
            $fullparam = 'omfull' . $index;
            $params[$codeparam] = '%' . $DB->sql_like_escape($value) . '%';
            $params[$fullparam] = '%' . $DB->sql_like_escape($value) . '%';
            $exists = 'EXISTS (SELECT 1'
                . ' FROM {local_outcomemap_qmap} omm'
                . ' JOIN {local_outcomemap_itemver} omv ON omv.id = omm.itemverid'
                . ' JOIN {local_outcomemap_item} omi ON omi.id = omv.itemid'
                . ' JOIN {local_outcomemap_fw} omf ON omf.id = omi.frameworkid'
                . ' WHERE omm.questionversionid = qv.id'
                . ' AND (' . $DB->sql_like('omi.code', ':' . $codeparam, false)
                . ' OR ' . $DB->sql_like($fullcode, ':' . $fullparam, false) . '))';
            $fragments[] = $jointype === \core\output\datafilter::JOINTYPE_NONE ? 'NOT ' . $exists : $exists;
        }
        $glue = $jointype === \core\output\datafilter::JOINTYPE_ANY ? ' OR ' : ' AND ';
